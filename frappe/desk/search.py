...
def sanitize_searchfield(searchfield):
    ...
    if SPECIAL_CHAR_PATTERN.search(searchfield):
        frappe.throw(_("Invalid Search Field {0}").format(
            searchfield), frappe.DataError)
...
def search_widget(
    ...
    if query and query.split(maxsplit=1)[0].lower() != "select":
        ...
        if query:
            ...
            formatted_fields = [
                f"`tab{meta.name}`.`{f.strip()}`" for f in fields]

            # Insert title field query after name
            if meta.show_title_field_in_link:
                formatted_fields.insert(
                    1, f"`tab{meta.name}`.{meta.title_field} as `label`")

            # In order_by, `idx` gets second priority, because it stores link count
            from frappe.model.db_query import get_order_by

            order_by_based_on_meta = get_order_by(doctype, meta)
            print("order_by_based_on_meta", order_by_based_on_meta)
            ...
            ignore_permissions = (
                ...
                if doctype == "DocType"
                    ...
                    and has_permission(
                        doctype,
                        ptype="select" if frappe.only_has_select_perm(
                            doctype) else "read",
            ...
            if meta.translated_doctype:
                ...
                values = (
                    ...
                    if any(
                        re.search(f"{re.escape(txt)}.*",
                                  _(cstr(value)) or "", re.IGNORECASE)
            ...
            values = sorted(
                values, key=lambda x: relevance_sorter(x, txt, as_dict))
...
def build_for_autosuggest(res: list[tuple], doctype: str) -> list[dict]:
    def to_string(parts):
        return ", ".join(
            unique(_(cstr(part)) if meta.translated_doctype else cstr(part)
...
def get_names_for_mentions(search_term):
    ...
    for mention_data in users_for_mentions + user_groups:
        ...
        mention_data["link"] = frappe.utils.get_url_to_form(
            "User Group" if mention_data.get(
                "is_group") else "User Profile", mention_data["id"]
        )
